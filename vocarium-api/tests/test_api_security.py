...
class UserCreationRaceTest(_A5DatabaseTestCase):
    def test_wal_race_converges_and_only_creator_seeds(self):
        ...
        self.assertEqual([kind for _thread_id, kind in creator_calls], ["voices"])
    ...
    def test_other_request_cannot_commit_failed_creators_partial_work(self):
        ...
        try:
            ...
            database._BUILTIN_SPEAKER_INSTRUCTS = PausingFailingInstructs(
                {"Interleave": ""}
            )
        ...
        self.assertEqual(str(first_errors[0]), "voice seed failed")
    ...
    def test_seed_failure_rolls_back_user_partial_seed_and_reuses_connection(self):
        ...
        class FailingInstructs(dict):
            def get(self, key, default=None):
                raise RuntimeError("voice seed failed")

        try:
            database._BUILTIN_SPEAKERS = [("Rollback", "female", "German")]
            database._BUILTIN_SPEAKER_INSTRUCTS = FailingInstructs({"Rollback": ""})
            with self.assertRaisesRegex(RuntimeError, "voice seed failed"):
        ...
        try:
            ...
            observer.close()
        self.assertEqual(state_after_failure, (0, 0))
...
class SeedTransactionOwnershipTest(_A5DatabaseTestCase):
    ...
    def test_public_seed_helpers_keep_one_argument_api(self):
        ...
        for helper in (self.database.seed_prebuilt_custom_voices,):
    ...
    def test_legacy_qwen_hosts_are_retired_but_repointed_ones_survive(self):
        """Ein auto-geseedeter Qwen-Host fliegt raus, ein umgehängter bleibt."""
        self.database.seed_prebuilt_custom_voices(self.user_id)
        qwen_voice = self.db.execute(
            "SELECT id FROM voices WHERE user_id=? AND source='custom' LIMIT 1",
            (self.user_id,),
        ).fetchone()[0]
        rows = [
            (f"host-vivian-{self.user_id}", "Vivian", qwen_voice),
            (f"host-serena-{self.user_id}", "Serena", "OmniVoice-Name"),
            (f"host-eigen-{self.user_id}", "Eigenbau", qwen_voice),
        ]
        for host_id, name, voice_id in rows:
            self.db.execute(
                "INSERT INTO hosts (id, user_id, name, voice_id) VALUES (?,?,?,?)",
                (host_id, self.user_id, name, voice_id),
            )
        removed = self.database._retire_legacy_qwen_hosts(self.db, self.user_id)
        survivors = {
            row[0]
            for row in self.db.execute(
                "SELECT id FROM hosts WHERE user_id=?", (self.user_id,)
            ).fetchall()
        }
        self.db.rollback()
        ...
        self.assertEqual(
            survivors,
            {f"host-serena-{self.user_id}", f"host-eigen-{self.user_id}"},
        )
