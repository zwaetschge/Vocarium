...
- Android client: native Kotlin app (package `ch.zwaetschge.vocarium`, 3.0.0-dev series) developed in `android-app-creator` project `242819cb-6367-4c34-bb0e-75fa70fc0b7f`; `NativeActivity` holds all screens, `NativePlaybackService` the queue player, `NativeApi` the authenticated transport. The repo mirrors its sources under `android/` via `scripts/sync-android.sh` (run it before committing app changes). Build/install only through the android-builder MCP tools; Gradle logs via `docs/implementation/android-native-client/buildlog.js`.
...
- **Podcast script pipeline (2026-09-18):** `podcast/analysis.py` runs three parallel analysis agents before the writer (Belege/facts, Gegenlesart/critique, Dramaturgie/dramaturgy) on `PODCAST_ANALYSIS_MODEL` (default `glm-5.3-flash`, via the user's active provider; `PODCAST_ANALYSIS_AGENTS=0` disables), formats them as a "Redaktionsdossier" into the writer prompt (`format_dossier()`), and afterwards runs a Lektorat pass (`review_script()`, `PODCAST_SCRIPT_REVIEW`, rewrites at most `PODCAST_REVIEW_MAX_SHARE`=40 % of segments, never speaker/type/order). `ScriptGenerator.generate(progress=…)` reports stages `analyzing`/`writing`/`reviewing` over the SSE route. Measured: short dialog 147 s total (58 s analysis, 63 s writing, 26 s review), review score 8/10 with 2 rewrites.
